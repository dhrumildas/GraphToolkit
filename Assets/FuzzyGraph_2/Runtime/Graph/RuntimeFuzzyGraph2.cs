...
namespace FuzzyGraph2.Runtime
{
    ...
    public sealed class RuntimeFuzzyGraph2 : ScriptableObject
    {
        ...
        {
            if (compiledEvents == null) throw new ArgumentNullException(nameof(compiledEvents));
            ...
            foreach (CompiledFuzzyEvent compiledEvent in compiledEvents)
            {
                if (compiledEvent == null)
                    throw new ArgumentException("The compiled graph contains a null event.", nameof(compiledEvents));

                if (string.IsNullOrWhiteSpace(compiledEvent.eventId))
                    throw new ArgumentException("Every compiled event requires an event ID.", nameof(compiledEvents));
                ...
                if (!eventIds.Add(compiledEvent.eventId))
                {
                    throw new ArgumentException(
                        $"More than one compiled event uses the ID '{compiledEvent.eventId}'.",
        ...
        public CompiledFuzzyEvent GetEvent(string eventId)
        {
            ...
            if (!TryGetEvent(eventId.Trim(), out CompiledFuzzyEvent compiledEvent))
                throw new KeyNotFoundException($"RuntimeFuzzyGraph2 does not contain an event named '{eventId}'.");
        ...
        private static Dictionary<string, FuzzyVariableDefinition> BuildVariables(CompiledFuzzyEvent compiledEvent)
        {
            if (compiledEvent.variables == null)
                throw new InvalidOperationException($"Event '{compiledEvent.eventId}' has no variable collection.");

            Dictionary<string, FuzzyVariableDefinition> variables =
                new Dictionary<string, FuzzyVariableDefinition>(StringComparer.Ordinal);

            foreach (CompiledFuzzyVariable compiledVariable in compiledEvent.variables)
            {
                if (compiledVariable == null)
                    throw new InvalidOperationException($"Event '{compiledEvent.eventId}' contains a null variable.");

                if (compiledVariable.sets == null)
                    throw new InvalidOperationException($"Variable '{compiledVariable.id}' has no set collection.");
                ...
                if (variables.ContainsKey(variable.Id))
                {
                    throw new InvalidOperationException(
                        $"Event '{compiledEvent.eventId}' contains more than one fuzzy variable named '{variable.Id}'.");
                }
        ...
        {
            if (compiledSet == null)
                throw new InvalidOperationException("A compiled fuzzy variable contains a null set.");

            switch (compiledSet.shape)
            {
                case FuzzySetShape.Low:
                    return FuzzySetDefinition.Low(
                        compiledSet.name, full: compiledSet.first, end: compiledSet.second);

                case FuzzySetShape.Range:
                    return FuzzySetDefinition.Range(
                        compiledSet.name, start: compiledSet.first, fullStart: compiledSet.second,
                        fullEnd: compiledSet.third, end: compiledSet.fourth);

                case FuzzySetShape.High:
                    return FuzzySetDefinition.High(
                        compiledSet.name, start: compiledSet.first, full: compiledSet.second);

                default:
                    throw new InvalidOperationException($"Unsupported compiled fuzzy-set shape: {compiledSet.shape}.");
            }
        }
            ...
            IReadOnlyDictionary<string, FuzzyVariableDefinition> variables)
        {
            if (compiledEvent.expressions == null)
                throw new InvalidOperationException($"Event '{compiledEvent.eventId}' has no expression collection.");
            ...
            {
                BuildExpression(index, compiledEvent.expressions, variables, expressions, buildStates);
            }
            ...
            byte[] buildStates)
        {
            if (expressionIndex < 0 || expressionIndex >= compiledExpressions.Count)
                throw new InvalidOperationException($"Expression index {expressionIndex} is outside the collection.");

            if (buildStates[expressionIndex] == 2) return builtExpressions[expressionIndex];

            if (buildStates[expressionIndex] == 1)
                throw new InvalidOperationException($"Expression at index {expressionIndex} contains a circular loop.");

            CompiledFuzzyExpression compiledExpression = compiledExpressions[expressionIndex];
            if (compiledExpression == null)
                throw new InvalidOperationException($"Expression at index {expressionIndex} is null.");
            ...
            switch (compiledExpression.kind)
            {
                case CompiledExpressionKind.FuzzyIs:
                    if (!variables.TryGetValue(compiledExpression.variableId, out FuzzyVariableDefinition variable))
                    {
                        throw new InvalidOperationException(
                            $"Expression {expressionIndex} references unknown fuzzy variable '{compiledExpression.variableId}'.");
                    }
                ...
                case CompiledExpressionKind.BoolEquals:
                    expression = FuzzyExpression.BoolEquals(
                        compiledExpression.variableId, compiledExpression.expectedBool);
                    break;

                case CompiledExpressionKind.IdEquals:
                    expression = FuzzyExpression.IdEquals(
                        compiledExpression.variableId, compiledExpression.expectedId);
                    break;

                case CompiledExpressionKind.NumberCompare:
                    expression = FuzzyExpression.NumberCompare(
                ...
                case CompiledExpressionKind.And:
                    expression = FuzzyExpression.And(BuildMultipleChildren(
                        expressionIndex, compiledExpression, compiledExpressions,
                        variables, builtExpressions, buildStates, "AND"));
                    break;

                case CompiledExpressionKind.Or:
                    expression = FuzzyExpression.Or(BuildMultipleChildren(
                        expressionIndex, compiledExpression, compiledExpressions,
                        variables, builtExpressions, buildStates, "OR"));
                    break;

                case CompiledExpressionKind.Not:
                    expression = FuzzyExpression.Not(BuildSingleChild(
                        expressionIndex, compiledExpression, compiledExpressions,
                        variables, builtExpressions, buildStates));
                    break;

                default:
                    throw new InvalidOperationException($"Unsupported compiled expression kind: {compiledExpression.kind}.");
            }
        ...
        private static IFuzzyExpression[] BuildMultipleChildren(
            ...
            string operatorName)
        {
            if (compiledExpression.childExpressionIndices == null || compiledExpression.childExpressionIndices.Count < 2)
                throw new InvalidOperationException($"{operatorName} expression {expressionIndex} needs at least 2 children.");

            IFuzzyExpression[] children = new IFuzzyExpression[compiledExpression.childExpressionIndices.Count];
            for (int index = 0; index < compiledExpression.childExpressionIndices.Count; index++)
            {
                children[index] = BuildExpression(
                    ...
                    compiledExpressions, variables, builtExpressions, buildStates);
            }
        ...
        private static IFuzzyExpression BuildSingleChild(
            ...
            byte[] buildStates)
        {
            if (compiledExpression.childExpressionIndices == null || compiledExpression.childExpressionIndices.Count != 1)
                throw new InvalidOperationException($"NOT expression {expressionIndex} requires exactly one child expression.");
            ...
            IReadOnlyList<IFuzzyExpression> expressions)
        {
            if (compiledEvent.rules == null)
                throw new InvalidOperationException($"Event '{compiledEvent.eventId}' has no rule collection.");
            ...
            foreach (CompiledSugenoRule compiledRule in compiledEvent.rules)
            {
                if (compiledRule == null)
                    throw new InvalidOperationException($"Event '{compiledEvent.eventId}' contains a null rule.");

                if (compiledRule.antecedentExpressionIndex < 0 || compiledRule.antecedentExpressionIndex >= expressions.Count)
                {
                    throw new InvalidOperationException(
                        $"Rule '{compiledRule.ruleId}' references invalid expression index {compiledRule.antecedentExpressionIndex}.");
                }
                ...
                if (!ruleIds.Add(rule.Id))
                    throw new InvalidOperationException($"Event '{compiledEvent.eventId}' has multiple rules named '{rule.Id}'.");
        ...
        private static NarrativeOutputMapping BuildOutputMapping(CompiledFuzzyEvent compiledEvent)
        {
            if (compiledEvent.outputBands == null)
                throw new InvalidOperationException($"Event '{compiledEvent.eventId}' has no output-band collection.");
            ...
            if (compiledEvent.fallbackBand == null)
                throw new InvalidOperationException($"Event '{compiledEvent.eventId}' has no authored fallback.");
        ...
        private static NarrativeOutputMapping.Band BuildOutputBand(CompiledOutputBand compiledBand)
        {
            if (compiledBand == null)
                throw new InvalidOperationException("The compiled event contains a null output band.");
