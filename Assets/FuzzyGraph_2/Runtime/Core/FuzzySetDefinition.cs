...
namespace FuzzyGraph2.Runtime
{
    public sealed class FuzzySetDefinition
    {
        ...
        public static FuzzySetDefinition High(
            string name,
            float start,
            float full)
        {
        ...
        public static FuzzySetDefinition Low(
            string name,
            float full,
            float end)
        {
            ...
            float end)
        {
            ...
            FuzzyMembership.ValidateRangeBounds(
                start,
                fullStart,
                fullEnd,
                end);
            ...
            switch (Shape)
            {
                ...
                default:
                    throw new InvalidOperationException(
                        $"Unsupported fuzzy-set shape: {Shape}.");
            }
        }
        ...
        private static void ValidateName(string name)
        {
            if (string.IsNullOrWhiteSpace(name))
            {
                throw new ArgumentException(
                    "A fuzzy set must have a readable name.",
                    nameof(name));
            }
        }
    }
